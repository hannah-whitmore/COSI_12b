...
public class SimulationDriver {
	...
	private static Scanner consoleRdr;
		
	/**
	 * Sets up and runs a RaceTrack using the helper method getSomeRaceCars() and getSomeFormulaOnes(). 
	 * @param args cmd line args not used 
	 * @see #getSomeRaceCars(), {@link #getSomeFormulaOnes()}
	 */
	public static void main(String[] args) {
		// set up a RaceTrack with an array of RaceCars read from user input using getSomeRaceCars() 
		RaceTrack track = new RaceTrack();
		System.out.println("Welcome to the Need for Speed Simulator!\n\nFor each car: enter the speed and strength separated by a space.\nIf you want to construct a default car, enter " + DEFAULT_STAT_VAL + " for the speed and strength.");
		track.setCars(getSomeRaceCars(), getSomeFormulaOnes());
	...
	public static RaceCar[] getSomeRaceCars() {
		consoleRdr = new Scanner(System.in);
		int numCars = -1;
	...
	public static FormulaOne[] getSomeFormulaOnes() {
		...
		FormulaOne[] cars = new FormulaOne[numCars];
		// for each car user will enter, loop until the user provides valid car initialization data (2 nonnegtive integers that are either both 0 or nonzero) 
		for (int i = 0; i < numCars; i++) {
			boolean carInvalid = true;
			do {
